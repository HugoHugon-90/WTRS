...
class FieldNotFoundError(Exception):
    ...
    def __str__(self):
        error = ErrorHelpers.error_printer(const.JSON_MANDATORY_FIELDS_NOT_FOUND_CODE, const.JSON_MANDATORY_FIELDS_NOT_FOUND_MESSAGE)
        if self.arg_len == 1:
...
class WrongInputConfigurationError(Exception):
    ...
    def __str__(self):
        error = ErrorHelpers.error_printer(const.INPUT_CONFIGURATION_ILL_DEFINED_CODE, const.INPUT_CONFIGURATION_ILL_DEFINED_MESSAGE)
